    ...
    function processSearchQuery(query) {
      ...
      for (const [provider, path] of Object.entries(vendorTypes)) {
        if (searchTerm === provider) {
          vendorType = path;
          console.log(`Found exact vendor match: ${vendorType} from provider: ${provider}`);
          break;
        }
        
        const providerPatterns = [
          new RegExp(`\\b${provider}\\b`, 'i'),
          new RegExp(`need\\s+a\\s+${provider}`, 'i'),
          new RegExp(`find\\s+a\\s+${provider}`, 'i'),
          new RegExp(`looking\\s+for\\s+a\\s+${provider}`, 'i'),
          new RegExp(`hire\\s+a\\s+${provider}`, 'i'),
          new RegExp(`get\\s+a\\s+${provider}`, 'i')
        ];
        
        for (const pattern of providerPatterns) {
          if (pattern.test(searchTerm)) {
            vendorType = path;
            console.log(`Found vendor match with pattern: ${vendorType} from provider: ${provider}`);
            break;
          }
        }
      ...
      if (vendorType) {
        ...
        console.log(`Setting URL to specific vendor: ${vendorUrl}`);
